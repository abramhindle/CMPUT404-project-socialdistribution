...
urlpatterns = [
    ...
    path('<path:author_id>/posts/<path:post_id>/comments/', CommentView.as_view(), name='comment_view'), #ORDER MATTERS!!!!!!! when dealing with path: params
    path('<path:author_id>/posts/<path:post_id>/', PostWithId.as_view(), name='post_with_id'),
    path('<path:author_id>/inbox/', InboxView.as_view(), name='inbox_view'),
